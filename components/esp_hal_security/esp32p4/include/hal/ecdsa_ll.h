...
#if HAL_CONFIG(CHIP_SUPPORT_MIN_REV) < 300
        // The hw_ver1 (rev <3.0) register layout only has a 1-bit curve-select
        // field with no slot for SECP384R1 - selecting it would silently
        // truncate to the wrong curve instead of erroring, so assert instead.
        HAL_ASSERT(false && "SECP384R1 not supported on ESP32-P4 rev <3.0 (hw_ver1 register layout has no P-384 curve select bit)");
        break;
#else
        REG_SET_FIELD(ECDSA_CONF_REG, ECDSA_ECC_CURVE, curve);
        break;
 ...
 *
 * Stock ESP-IDF disables the ESP32-P4 hardware ECDSA accelerator below
 * eco5 (major 3, minor 0+) because those revisions have a known
 * side-channel key-extraction vulnerability. This vendored override
 * force-enables it unconditionally: only safe when the application does
 * not need ECDSA to resist physical/side-channel attacks. See this
 * component's README.md for details.
 */
