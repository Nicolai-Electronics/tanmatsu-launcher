...
static const char* AUTOSTART_KEY = "autostart";
...
esp_err_t app_autostart_get(char* out_slug) {
    ...
    size_t size = 0;
    res         = nvs_get_str(handle, AUTOSTART_KEY, NULL, &size);
    if (res != ESP_OK) {
        nvs_close(handle);
        return res;
    }
    ...
    res = nvs_get_str(handle, AUTOSTART_KEY, out_slug, &size);
    nvs_close(handle);
    return res;
}
...
esp_err_t app_autostart_set(const char* slug) {
    ...
    if (slug == NULL) {
        res = nvs_erase_key(handle, AUTOSTART_KEY);
    } else {
        res = nvs_set_str(handle, AUTOSTART_KEY, slug);
    }
